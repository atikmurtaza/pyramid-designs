...
At £0, no credible supported automated malware scanner exists for Hostinger-managed Node.js. The owner-approved MVP model keeps files `QUARANTINED_UNSCANNED` and unavailable to ordinary hiring review until a named authorised security reviewer performs the documented managed-endpoint procedure and records an immutable clean outcome with matching hash/tool evidence. MIME/header/extension checks are validation controls only. Production intake remains gated until that procedure and its residual-risk acceptance are documented and verified.
